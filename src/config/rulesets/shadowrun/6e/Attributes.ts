...
export const SPECIAL_ATTRIBUTE_OPTIONS: AttrName[] = ["edge", "magic"];

export const ALL_ATTRIBUTES = ATTRIBUTE_OPTIONS.concat(
...
export class AttributesBuilder implements Builder<Attributes> {
  ...
  areMaxesInError(): boolean {
    ...
    return ALL_ATTRIBUTES.some((attribute: AttrName) => {
      ...
      this.error =
        maxes.length > 1
          ? `Two attributes [${maxes.join(
              ","
            )}] cannot be set to racial max during character creation.`
          : "";
