...
import { Attributes, AttributesBuilder, AttrName, ATTRIBUTE_OPTIONS, SPECIAL_ATTRIBUTE_OPTIONS, ALL_ATTRIBUTES } from "../config/rulesets/shadowrun/6e/Attributes";
...
class NewAttributes extends React.Component<Props, State> {
    ...
    constructor(props: Props) {
        ...
        if (props.priority.Attributes != null && props.priority.Metatype != null) {
            ...
            builder = ALL_ATTRIBUTES.map(option => builder.set(option, 1)).pop() as AttributesBuilder;
            this.state = {
                builder,
                totalPoints: (props.priority.Attributes.value as number) + (props.priority.Metatype.value as number)
            };
        } else
            throw new Error("Somehow you got here without having Attributes or Metatype set.");
    }
    
    handleChanged = (attribute: AttrName, _: number, delta: number) => {
        this.setState({
            builder: delta > 0 ? this.state.builder.increment(attribute) : this.state.builder.decrement(attribute),
            totalPoints: this.state.totalPoints - delta
        }, () => {
            this.props.attributesFinished((this.state.totalPoints) === 0 && this.state.builder.isReady() ? this.state.builder.build() : undefined)
        });
    }
