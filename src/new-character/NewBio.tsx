...
class NewBio extends React.Component<Props, State> {
  ...
  checkIfDone() {
    this.props.bioFinished(
      this.state.bioBuilder.isReady()
        ? this.state.bioBuilder.build()
        : undefined
    );
  }
  ...
  updateMeta = (value: string) => {
    ...
    if (type !== undefined) {
      this.setState({
        bioBuilder: this.state.bioBuilder
          .setMetatype(type)
          .setHeight(type.averageHeight)
          .setWeight(type.averageWeight),
      });
    }
    this.checkIfDone();
  };
  ...
  updateName = (event: ChangeEvent<any>) => {
    if (event.target?.value !== undefined) {
      this.setState({
        bioBuilder: this.state.bioBuilder.setName(event.target.value),
      });
    }
    this.checkIfDone();
  };
  ...
  updateEthnicity = (event: ChangeEvent<any>) => {
    if (event.target?.value !== undefined) {
      this.setState({
        bioBuilder: this.state.bioBuilder.setEthnicity(event.target.value),
      });
    }
    this.checkIfDone();
  };
  ...
  updateSex = (_: any, element: any) => {
    if (element.value !== undefined) {
      this.setState({
        bioBuilder: this.state.bioBuilder.setSex(element.value),
      });
    }
    this.checkIfDone();
  };
  ...
  updateAge = (event: ChangeEvent<any>) => {
    if (event.target?.value !== undefined) {
      this.setState({
        bioBuilder: this.state.bioBuilder.setAge(event.target.value),
      });
    }
    this.checkIfDone();
  };
  ...
  updateHeight = (event: ChangeEvent<any>) => {
    if (event.target?.value !== undefined) {
      this.setState({
        bioBuilder: this.state.bioBuilder.setHeight(event.target.value),
      });
    }
    this.checkIfDone();
  };
  ...
  updateWeight = (event: ChangeEvent<any>) => {
    if (event.target?.value !== undefined) {
      this.setState({
        bioBuilder: this.state.bioBuilder.setWeight(event.target.value),
      });
    }
    this.checkIfDone();
  };
          ...
          <div className="two fields">
            <div className="field">
              ...
              <Select
                ...
                onChange={(_, { value }) => {
                  this.updateMeta(value as string);
                }}
                options={Metatypes.types.map((type) => {
                  return {
                    key: type.name,
                    value: type.name,
                    text: type.name,
