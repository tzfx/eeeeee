...
class NewPriorities extends React.Component<Props, State> {
  ...
  isInvalid = (): boolean => {
    const ranks = new Set();
    Object.values(this.state).forEach((val: any) => {
  ...
  selectPriority = (type: string, letter: Letter) => {
    ...
    this.setState(state, () =>
      this.props.prioritiesFinished(this.isInvalid() ? undefined : this.state)
    );
  };
                  ...
                  if (k === "Magic") {
                    return (
                      <td
                        ...
                        align="right"
                      >
                        {Object.entries((PriorityOptions as any)[k][letter])
                          .map((v) => {
                            return `${v.join(": ")}`;
                          })
                          .join(", ")}
