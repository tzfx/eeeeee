...
class NewSkills extends React.Component<Props, State> {
    ...
    checkMaxes(): boolean {
        const maxed: string[] = [];
        return Array.from(this.state.selectedSkills.values())
            .every(skill => {
                if (skill.rank === 6) {
                    maxed.push(skill.name);
                }
                this.setState({
                    error: (maxed.length < 2) ? `You cannot have 2 skills at max [${maxed.join(", ")}] at chargen.` : ""
                });
    ...
    selectSkill = (skill: Skill) => {
        const delta = 
            (!this.state.selectedSkills.has(skill.name) || ((this.state.selectedSkills.get(skill.name) as Skill).rank < skill.rank))
            ? 1 : -1;
        this.setState({
            points: this.state.points - delta,
            selectedSkills: this.state.selectedSkills.set(skill.name, skill)
        }, () => {
            this.props.skillsFinished(this.state.points === 0 && this.checkMaxes() ? this.state.selectedSkills : undefined);
        });
    }
            ...
            <Container>
                ...
                <Grid columns="2" relaxed="very" >
                        ...
                        <CardGroup>
                            {
                                Array.from(this.state.skillsList.values())
                                    ...
                                    .map((skill) => {
                                        return (<SkillDisplay disableUp={this.state.points === 0} selected={false} select={(skill) => this.selectSkill(skill)} key={skill.name} skill={skill}></SkillDisplay>)
                                    })
                            }
                        </CardGroup>
                    </Grid.Column>
                    <Grid.Column>
                        <CardGroup>
                            {
                                Array.from(this.state.selectedSkills.values())
                                .map((skill) => {
                                    return (<SkillDisplay disableUp={this.state.points === 0} selected={true} select={(skill) => this.deselectOrUpdateSkill(skill)} key={skill.name} skill={skill}></SkillDisplay>)
                                })
                            }
                        </CardGroup>
                    </Grid.Column>
